...
export default function WorkspaceLayout({
  ...
  const switchRoleMutation = trpc.profile.switchRole.useMutation({
    onSuccess: (data) => {
      toast.success(`Switched role to ${roleDisplayNames[data.role]}`);
      const targetRoute = roleRoutes[data.role] || "/dashboard/citizen";
  ...
  const handleRoleSelect = (targetRole: "citizen" | "asha_cho" | "doctor" | "facility_staff" | "administrator") => {
    ...
    <div className="min-h-screen bg-[#f3f5f8] dark:bg-[#0b0f14] text-[#15181b] dark:text-[#f1f5f9] flex flex-col antialiased selection:bg-[#6c9db9]/20">
      ...
      <header className="sticky top-0 z-40 border-b border-black/[0.06] dark:border-white/[0.08] bg-white/85 dark:bg-[#121820]/85 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-[1600px] items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          {/* Role Switcher Pill & Quick Navigation */}
          <div className="hidden items-center gap-3 md:flex">
            <div className="relative">
              <button
                onClick={() => setRoleSwitcherOpen(!roleSwitcherOpen)}
                className="flex items-center gap-2 rounded-full border border-black/10 dark:border-white/10 bg-[#f7f9fa] dark:bg-slate-800/80 px-3.5 py-1.5 text-xs font-semibold text-slate-800 dark:text-slate-200 shadow-xs transition hover:bg-white dark:hover:bg-slate-800"
              >
              ...
              {roleSwitcherOpen && (
                <div className="absolute left-0 mt-2 w-60 rounded-2xl border border-black/10 dark:border-slate-800 bg-white dark:bg-slate-900 p-2 shadow-xl z-50 animate-in fade-in zoom-in-95">
                  <div className="px-3 py-1.5 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Switch Workspace Role</div>
                  {(["citizen", "asha_cho", "doctor", "facility_staff", "administrator"] as const).map((r) => (
                    <button
                      key={r}
                    ...
                    >
                      ...
                      {role === r && <CheckCircle2 className="h-3.5 w-3.5 text-emerald-400" />}
                    </button>
                  ))}
                </div>
              )}
        ...
        {/* Mobile Navigation Drawer / Role Dropdown */}
        {mobileMenuOpen && (
          <div className="border-t border-black/5 dark:border-white/10 bg-white dark:bg-[#121820] p-4 lg:hidden">
            <div className="mb-3 flex items-center justify-between">
              <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase">Switch Workspace</span>
              <Badge className="dark:bg-slate-800 dark:text-slate-200">{roleDisplayNames[role]}</Badge>
            </div>
